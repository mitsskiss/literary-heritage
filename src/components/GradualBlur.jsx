...
const mergeConfigs = (...configs) =>
  configs.reduce((accumulator, config) => ({ ...accumulator, ...config }), {});
...
function GradualBlur(props) {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let index = 1; index <= config.divCount; index += 1) {
      ...
      const point1 = Math.round((increment * index - increment) * 10) / 10;
      const point2 = Math.round(increment * index * 10) / 10;
      const point3 = Math.round((increment * index + increment) * 10) / 10;
      const point4 = Math.round((increment * index + increment * 2) * 10) / 10;

      let gradient = `transparent ${point1}%, black ${point2}%`;
      if (point3 <= 100) gradient += `, black ${point3}%`;
      if (point4 <= 100) gradient += `, transparent ${point4}%`;
  ...
  useEffect(() => {
    if (isVisible && animated === "scroll" && onAnimationComplete) {
      ...
      const timeoutId = setTimeout(() => onAnimationComplete(), ms);
